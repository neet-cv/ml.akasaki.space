...
查询向量是一个滤波或变换，用于在输入中筛选出更有用的信息。为了实现选择关键的信息来进行处理，我们引入了一个与任务相关的查询向量$Q$，并且使用一个打分函数来计算不同信息向量与查询向量的相关性。
...
打分函数通常情况下是用于计算**每个输入向量和查询向量之间的相关性**的。常见的打分函数计算方式有：
...
上式中${\alpha}_{i}$称为注意力分布，$s(x_{i},q)$是注意力的打分函数。所谓打分函数，就是计算每个输入量和查询向量之间的相关性的函数。
